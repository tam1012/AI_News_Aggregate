...
export async function resolveSourceUrl(rawUrl: string, fetcher: ResolveFetch = defaultFetch): Promise<SourceDetectResult> {
  ...
  if (isYoutubeUrl(normalized)) {
    ...
    result.warnings.push('YouTube sources are not supported in this deployment yet.');
    result.preview = { title: 'YouTube', description: 'YouTube source support is not enabled yet', rss_count: 0 };
    return result;
  }
